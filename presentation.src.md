...
```bash +exec_replace
mermaid-ascii -x 12 -y 3 -f - <<'MMD'
graph TB
classDef person color:#9ece6a
classDef game color:#7aa2f7
...
graph LR
classDef src color:#9ece6a
classDef game color:#7aa2f7
classDef sink color:#f7768e
source:::src --> Alice
source --> Bob
source --> Carol
Alice --> DnD[D&D]:::game
...
classDef sink color:#f7768e
classDef act color:#e0af68
classDef done color:#9ece6a
source:::src --> Alice:::act
source --> Bob:::done
Alice --> DnD[D&D 1]:::done
Bob --> DnD
DnD -->|undo| Bob
Bob --> CoC[CoC 1]:::act
DnD --> sink:::sink
