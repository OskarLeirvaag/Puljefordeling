...
// Weight returns the priority weight for an edge with the given attributes.
// Exported for tooling/demos that want to show the band structure; the solver
// itself uses the unexported adjustScore.
func Weight(score model.Score, satisfied, neverSeated, isDM bool, misses int) int {
	return adjustScore(score, satisfied, neverSeated, isDM, misses)
}

// ParticipationBonus is the value (in weight units) the solver places on
// seating one more attendee. Exported for tooling/demos.
const ParticipationBonus = participationBonus
